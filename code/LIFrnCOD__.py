...
def args_parser(args):
    ...
    return name, run, rho, xi, with_noise, pp, setting, n_clusters, Max_iters
...
class LIFrnCOD:
    ...
    def __init__(self, y, p, labels, rho, xi, n_clusters):
        super(LIFrnCOD, self).__init__()
        self.y = y
        self.p = p
        self.labels = labels
        self.rho = rho
    ...
    def select_neighbours(self, p, node):
    ...
    def compute_gain(self, y, p, rho, xi, indices):
    ...
    def phase_one(self, y, p, labels, rho, xi, greedy):
        ...
        for node in range(y.shape[0]):
            ...
            gain_old = self.compute_gain(y=y, p=p, rho=rho, xi=xi, indices=[node])
            # print("old gain:", gain_old)
            neighbours = self.select_neighbours(p=p, node=node)
            ...
            for neighbour in neighbours:
                gain_new = self.compute_gain(y=y, p=p, rho=rho, xi=xi, indices=[node, neighbour])
    ...
    def phase_two(self, y_original, p_original, labels, directed=False):
    ...
    def run_louvain(self, y, p, labels, rho, xi, n_clusters, max_iters=100):
        ...
        while f1 is True:
            ...
            labels_new = self.phase_one(y=y, p=p, labels=labels, rho=rho, xi=xi, greedy=True)
            ...
            if list(labels.values()) == list(labels_old.values()):
                ...
                y_agg, p_agg, updated_labels = self.phase_two(y_original=y_original, p_original=p_original,
...
if __name__ == '__main__':
    ...
    name, run, rho, xi, with_noise, pp, setting_, n_clusters, max_iters = args_parser(args)

    lifcod = LIFrnCOD.run_louvain
                ...
                for setting, repeats in DATA.items():
                    ...
                    if str(setting) == setting_:
                        ...
                        for repeat, matrices in repeats.items():
                            ...
                            # Quantitative case
                            if name.split('(')[0][-1] == 'Q':
                                _, _, y_z, _, y_rng, _, = preprocess_y(y_in=y,
                                                                       data_type='Q')

                                if with_noise == 1:
                                    y_n, _, y_n_z, _, y_n_rng, _, = preprocess_y(y_in=y_n,
                                                                                 data_type='Q')
                            ...
                            if name.split('(')[0][-1] == 'C':
                                ...
                                y, _, y_z, _, y_rng, _, = preprocess_y(y_in=y_onehot,
                            ...
                            if name.split('(')[0][-1] == 'M':
                                ...
                                _, _, y_z_q, _, y_rng_q, _, = preprocess_y(y_in=y[:, :v_q],
                                                                           data_type='Q')
                                ...
                                # for WITHOUT follow-up rescale y_onehot and for
                                # "WITH follow-up" y_onehot should be rep_laced with Y
                                _, _, y_z_c, _, y_rng_c, _, = preprocess_y(y_in=y[:, v_q:],
                                                                           data_type='C')  # y_onehot
                                ...
                                if with_noise == 1:
                                    ...
                                    _, _, y_n_z_q, _, y_n_rng_q, _, = preprocess_y(y_in=y_n[:, :v_q],
                                                                                   data_type='Q')
                                    ...
                                    _, _, y_n_z_, _, y_n_rng_, _, = preprocess_y(y_in=y_n[:, v_qn:],
                                                                                 data_type='Q')
